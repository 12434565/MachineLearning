...
features = [
    "cpg_density"
    ,"tata_box_score" # 0.556
    ,"promoter_gc_frac" # 0.541
    , "h3k4me3_signal"
    , "atac_accessibility"
    , "tf_motif_count"
]
...
input_train_tensors = torch.tensor(input_train.values).float()
input_test_tensors = torch.tensor(input_test.values).float()

label_train_tensor = torch.tensor(label_train, dtype=torch.long)

train_dataset = TensorDataset(input_train_tensors, label_train_tensor)
...
class MultipleInsOuts(L.LightningModule):
    def __init__(self):
        ...
        in_f_input_to_hidden1 = 6
        out_f_input_to_hidden1 = 2
        out_features_hidden2_to_output = 2 # fixed
        out_features_hidden1_to_hidden2 = 2

        self.input_to_hidden1 = nn.Linear(in_features=in_f_input_to_hidden1, out_features=out_f_input_to_hidden1, bias=True)
        self.hidden1_to_hidden2 = nn.Linear(in_features=out_f_input_to_hidden1, out_features=out_features_hidden1_to_hidden2, bias=True)
        self.hidden2_to_output = nn.Linear(in_features=out_features_hidden1_to_hidden2, out_features=out_features_hidden2_to_output, bias=True)
        # self.hidden_to_output = nn.Linear(in_features=2, out_features=2, bias=True)
        self.loss = nn.CrossEntropyLoss()
    def forward(self, input):
        x = self.input_to_hidden1(input)
        x = F.relu(x)
    ...
    def configure_optimizers(self):
        learnr = 0.01
        return Adam(self.parameters(), lr=learnr)
...
trainer = L.Trainer(max_epochs=epoches) # Before, max_epochs=10, so, by setting it to 100, we're adding 90 more.
...
"""
normalized_values = (sample - min_vals_in_input_train) / (max_vals_in_input_train - min_vals_in_input_train)
